...
def main():
    ...
    with tf.Session() as sess:
        ...
        plt.figure()
        plt.gray()
        conv_1_out_=np.array(conv_1_out)
        for i in range(50):
            plt.subplot(5,10, i+1); plt.axis('off'); plt.imshow(conv_1_out_[0,:,:,i])

        plt.figure()
        plt.gray()
        pool_1_=np.array(pool_1)
        for i in range(50):
            plt.subplot(5,10,i+1); plt.axis('off'); plt.imshow(pool_1_[0,:,:,i])

        plt.figure()
        plt.gray()
        conv_2_out_=np.array(conv_2_out)
        for i in range(60):
            plt.subplot(6,10, i+1); plt.axis('off'); plt.imshow(conv_2_out_[0,:,:,i])

        plt.figure()
        plt.gray()
        pool_2_=np.array(pool_2)
        for i in range(60):
