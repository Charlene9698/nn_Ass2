...
def main():
    ...
    with tf.Session() as sess:
        ...
        plt.figure()
        plt.gray()
        conv_2_out=np.array(conv_2_out)
        for i in range(60):
            plt.subplot(6,10, i+1); plt.axis('off'); plt.imshow(conv_2_out[0,:,:,i])

        plt.figure()
        plt.gray()
        pool_2=np.array(pool_2)
        for i in range(60):
            plt.subplot(6,10,i+1); plt.axis('off'); plt.imshow(pool_2[0,:,:,i])
        plt.show()
